...
outcomes_hospitalisations_raw%>%filter(outcomes_hosp_description=="cvd_hosp") %>% distinct(CODE)
outcomes_hospitalisations_raw_cvd%>%filter(outcomes_hosp_description=="cvd_hosp") %>% distinct(CODE)

outcomes_hospitalisations_raw%>%filter(CODE=="I500")%>%distinct(outcomes_hosp_description)
...
```{r, outcomesDf, warning=FALSE, error=FALSE, message=FALSE, eval=TRUE, echo=FALSE}
...
```{r, outcomesDfDeath, warning=FALSE, error=FALSE, message=FALSE, eval=TRUE, echo=FALSE}
...
# (test %>% ungroup()) == (test_cvd%>%ungroup())
# 
# f1=test%>%filter(PERSON_ID=="IJIVQ5XK4B8TPJD")
# f2=test_cvd%>%filter(PERSON_ID=="IJIVQ5XK4B8TPJD")
# 
# outcomes_hosp_prepared %>% group_by(outcomes_hosp_description) %>% count()
# outcomes_hosp_prepared_cvd %>% group_by(outcomes_hosp_description) %>% count()
...
```{r, Hosp_Working, warning=FALSE, error=FALSE, message=FALSE, eval=TRUE, echo=FALSE}
...
```{r, Hosp_Working_cvd, warning=FALSE, error=FALSE, message=FALSE, eval=TRUE, echo=FALSE}
...
testing=test%>%filter(CODE_4=="I509")
...
OPERATION_YEAR) %>%
  ...
  mutate(Ethnicity=factor(Ethnicity,
                                           levels=c("White",
                                                    ...
                                                    "Black or Black British",
                                                    "Mixed or Multiple ethnic groups"))) %>%
  left_join(outcomes_hosp_prepared_cvd) %>%
  mutate(outcomes_time_to_hosp = as.numeric(DOH-OPERATION_DATE))
...
kableExtra::kbl(
  ...
  add_header_above(header = c(" " = 2, "CVD Hospitalisation" = 2, " "=1,"All-cause Hospitalisation" = 2))%>%
  kable_styling(latex_options = c("scale_down"))
...
outcomes_hosp_df_ds_cvd_savr %>% 
    hr_plot(dependent_ds_cvd_model0, explanatory_model0,
            plot_opts=list(geom_point(aes(size = Total), shape=22, fill="#F51344")),
            prefix="SAVR - CVD Hospitalisation ",
            table_text_size = 6,title_text_size = 20
            )
...
outcomes_hosp_df_ds_cvd_savr_t %>% 
    hr_plot(dependent_ds_cvd_model3, explanatory_model3,
            plot_opts=list(geom_point(aes(size = Total), shape=22, fill="#F51344")),
            prefix="SAVR - CVD Hospitalisaion ",
            table_text_size = 6,title_text_size = 20
            )
...
outcomes_hosp_df_ds_cvd_tavi %>% 
    hr_plot(dependent_ds_cvd_model0, explanatory_model0,
            plot_opts=list(geom_point(aes(size = Total), shape=22, fill="#F51344")),
            prefix="TAVI - CVD Hospitalisation ",
            table_text_size = 6,title_text_size = 20
            )
...
outcomes_hosp_df_ds_cvd_tavi_t %>% 
    hr_plot(dependent_ds_cvd_model3, explanatory_model3,
            plot_opts=list(geom_point(aes(size = Total), shape=22, fill="#F51344")),
            prefix="TAVI - CVD Hospitalisaion ",
            table_text_size = 6,title_text_size = 20
            )
...
test1_cvd %>%
  ...
  filter(!(events_total==2 & event_no==2 & outcomes_hosp_description=="non_cvd_hosp")) %>%
  group_by(PERSON_ID) %>% mutate(events_total = n()) %>% ungroup()
  filter(PERSON_ID=="002EEBSVI4CIS8J")
  # now everyone that has 1 event will have either cvd or non cvd (CVD counted in CVD and both counted in All Cause)
  # and if someone has 2 events they had a non cvd first then CVD - in the All Cause cause event no 1 is used but in the CVD case then event no 1 is ignored and event 2 is used
...
outcomes_hosp_prepared %>% filter(PERSON_ID=="002EEBSVI4CIS8J")
outcomes_hosp_prepared_cvd %>% filter(PERSON_ID=="002EEBSVI4CIS8J")

hp=test1_cvd %>% filter(PERSON_ID=="002EEBSVI4CIS8J")

outcomes_hospitalisations_raw %>% filter(outcomes_hosp_description=="cvd_hosp") %>%
  distinct(CODE)
...
ds_survival_model2 %>% as.data.frame()
os_survival_model2
...
ds_survival_model2 %>% as.data.frame() %>%
  ...
  mutate(B="",`CVD Death`="") %>%
  cbind(
    os_survival_model2 %>% as.data.frame() %>%
  remove_rownames() %>%
  mutate(`All-cause Death`="") %>%
  select("n "=all,`HR (95% CI, p-value) `="HR (multivariable)",`All-cause Death`)
  ) %>%
  select(B,C=" ",n=all,`HR (95% CI, p-value)`="HR (multivariable)",`CVD Death`,`HR (95% CI, p-value) `,`All-cause Death`) %>%
  filter(C%in%c("White","Asian or Asian British","Black or Black British","Mixed or Multiple ethnic groups"))
) %>%
...
ds_survival_model2 %>% as.data.frame() %>%
  ...
  select(B,C=" ",n=all,`HR (95% CI, p-value)`="HR (multivariable)",`CVD Death`,`HR (95% CI, p-value) `,`All-cause Death`) %>%
  filter(C%in%c("Mean (SD)"))
)
  ) %>%
  
  mutate(C=ifelse(C!="",paste0(C,"\n"),C)) %>%
  mutate(C=ifelse(C=="",B,C)) %>%
  mutate(
    est_gp1 = str_extract(`HR (95% CI, p-value)`, "^\\d+\\.\\d+"),
    low_gp1 = str_extract(`HR (95% CI, p-value)`, "(?<=\\()[0-9]+\\.[0-9]+"),
    high_gp1 = str_extract(`HR (95% CI, p-value)`, "(?<=-)[0-9]+\\.[0-9]+")
  ) %>%
  mutate(
    est_gp2 = str_extract(`HR (95% CI, p-value) `, "^\\d+\\.\\d+"),
    low_gp2 = str_extract(`HR (95% CI, p-value) `, "(?<=\\()[0-9]+\\.[0-9]+"),
    high_gp2 = str_extract(`HR (95% CI, p-value) `, "(?<=-)[0-9]+\\.[0-9]+")
  )
...
`All-cause Death`=c("")
  ...
  rbind(
ds_survival_model4 %>% as.data.frame() %>%
  remove_rownames() %>%
  mutate(B="",`CVD Death`="") %>%
  cbind(
    os_survival_model4 %>% as.data.frame() %>%
  remove_rownames() %>%
  mutate(`All-cause Death`="") %>%
  select("n "=all,`HR (95% CI, p-value) `="HR (multivariable)",`All-cause Death`)
  ) %>%
...
`All-cause Death`=c("")
  ...
  rbind(
    ds_survival_model4 %>% as.data.frame() %>%
      mutate(`Overall survival`=ifelse(`Overall survival`=="",NA,`Overall survival`)) %>%
        fill("Overall survival") %>%
  filter(`Overall survival` == "Deprivation") %>%
  remove_rownames() %>%
  mutate(B="",`CVD Death`="") %>%
  cbind(
    os_survival_model4 %>% as.data.frame() %>%
...
ds_survival_model4 %>% as.data.frame() %>%
      mutate(`Overall survival`=ifelse(`Overall survival`=="",NA,`Overall survival`)) %>%
        fill("Overall survival") %>%
  filter(`Overall survival` == "Heart_Failure") %>%
  remove_rownames() %>%
  mutate(B="",`CVD Death`="") %>%
  cbind(
    os_survival_model4 %>% as.data.frame() %>%
...
ds_survival_model4 %>% as.data.frame() %>%
      mutate(`Overall survival`=ifelse(`Overall survival`=="",NA,`Overall survival`)) %>%
        fill("Overall survival") %>%
  filter(`Overall survival` == "Hypertension") %>%
  remove_rownames() %>%
  mutate(B="",`CVD Death`="") %>%
  cbind(
    os_survival_model4 %>% as.data.frame() %>%
...
ds_survival_model4 %>% as.data.frame() %>%
      mutate(`Overall survival`=ifelse(`Overall survival`=="",NA,`Overall survival`)) %>%
        fill("Overall survival") %>%
  filter(`Overall survival` == "Asthma_COPD") %>%
  remove_rownames() %>%
  mutate(B="",`CVD Death`="") %>%
  cbind(
    os_survival_model4 %>% as.data.frame() %>%
...
ds_survival_model4 %>% as.data.frame() %>%
      mutate(`Overall survival`=ifelse(`Overall survival`=="",NA,`Overall survival`)) %>%
        fill("Overall survival") %>%
  filter(`Overall survival` == "Cerebrovascular_Disease") %>%
  remove_rownames() %>%
  mutate(B="",`CVD Death`="") %>%
  cbind(
    os_survival_model4 %>% as.data.frame() %>%
...
ds_survival_model4 %>% as.data.frame() %>%
      mutate(`Overall survival`=ifelse(`Overall survival`=="",NA,`Overall survival`)) %>%
        fill("Overall survival") %>%
  filter(`Overall survival` == "Coronary_Artery_Disease") %>%
  remove_rownames() %>%
  mutate(B="",`CVD Death`="") %>%
  cbind(
    os_survival_model4 %>% as.data.frame() %>%
...
ds_survival_model4 %>% as.data.frame() %>%
      mutate(`Overall survival`=ifelse(`Overall survival`=="",NA,`Overall survival`)) %>%
        fill("Overall survival") %>%
  filter(`Overall survival` == "Treatment_Time") %>%
  remove_rownames() %>%
  mutate(B="",`CVD Death`="") %>%
  cbind(
    os_survival_model4 %>% as.data.frame() %>%
  ...
  mutate(C=ifelse(C=="",B,C)) %>%
  mutate(
    est_gp3 = str_extract(`HR (95% CI, p-value)`, "^\\d+\\.\\d+"),
    low_gp3 = str_extract(`HR (95% CI, p-value)`, "(?<=\\()[0-9]+\\.[0-9]+"),
    high_gp3 = str_extract(`HR (95% CI, p-value)`, "(?<=-)[0-9]+\\.[0-9]+")
  ) %>%
  mutate(
    est_gp4 = str_extract(`HR (95% CI, p-value) `, "^\\d+\\.\\d+"),
    low_gp4 = str_extract(`HR (95% CI, p-value) `, "(?<=\\()[0-9]+\\.[0-9]+"),
    high_gp4 = str_extract(`HR (95% CI, p-value) `, "(?<=-)[0-9]+\\.[0-9]+")
  )


col_names <- names(model2_df)

# Step 2: Create an initial empty data frame with the same column names as model4_df
initial_data <- tibble(.rows = 0, !!!set_names(vector("list", length(col_names)), col_names))

# Step 3: Create a new data frame with 16 blank rows and the same column names as model4_df
new_rows <- tibble(!!!set_names(replicate(length(col_names), rep("", 26), simplify = FALSE), col_names))



model2_df %>% rbind(new_rows)
my_df = model4_df %>% cbind((model2_df %>% rbind(new_rows) %>% rename_with(~ paste0("X_", .)))) %>%
  mutate(`HR (95% CI, p-value)`=paste0(`HR (95% CI, p-value)`,"\n",`X_HR (95% CI, p-value)`)) %>%
  mutate(`HR (95% CI, p-value) `=paste0(`HR (95% CI, p-value) `,"\n",`X_HR (95% CI, p-value) `)) %>%
  select(C,n,`HR (95% CI, p-value)`,`CVD Death`,`HR (95% CI, p-value) `,`All-cause Death`,
         est_gp1=X_est_gp1,low_gp1=X_low_gp1,high_gp1=X_high_gp1,est_gp2=X_est_gp2,low_gp2=X_low_gp2,high_gp2=X_high_gp2,
         est_gp3,low_gp3,high_gp3, est_gp4,low_gp4,high_gp4
         ) %>%
  
  mutate(est_gp1=ifelse(est_gp1=="",NA,est_gp1)) %>%
  mutate(est_gp2=ifelse(est_gp2=="",NA,est_gp2)) %>%
  mutate(est_gp3=ifelse(est_gp3=="",NA,est_gp3)) %>%
  mutate(est_gp4=ifelse(est_gp4=="",NA,est_gp4)) %>%
  
  mutate(low_gp1=ifelse(low_gp1=="",NA,low_gp1)) %>%
  mutate(low_gp2=ifelse(low_gp2=="",NA,low_gp2)) %>%
  mutate(low_gp3=ifelse(low_gp3=="",NA,low_gp3)) %>%
  mutate(low_gp4=ifelse(low_gp4=="",NA,low_gp4)) %>%
  
  mutate(high_gp1=ifelse(high_gp1=="",NA,high_gp1)) %>%
  mutate(high_gp2=ifelse(high_gp2=="",NA,high_gp2)) %>%
  mutate(high_gp3=ifelse(high_gp3=="",NA,high_gp3)) %>%
  mutate(high_gp4=ifelse(high_gp4=="",NA,high_gp4)) %>%
  
  mutate(est_gp1=as.numeric(est_gp1)) %>%
  mutate(est_gp2=as.numeric(est_gp2)) %>%
  mutate(est_gp3=as.numeric(est_gp3)) %>%
  mutate(est_gp4=as.numeric(est_gp4)) %>%
  
  mutate(low_gp1=as.numeric(low_gp1)) %>%
  mutate(low_gp2=as.numeric(low_gp2)) %>%
  mutate(low_gp3=as.numeric(low_gp3)) %>%
  mutate(low_gp4=as.numeric(low_gp4)) %>%
  
  mutate(high_gp1=as.numeric(high_gp1)) %>%
  mutate(high_gp2=as.numeric(high_gp2)) %>%
  mutate(high_gp3=as.numeric(high_gp3)) %>%
  mutate(high_gp4=as.numeric(high_gp4)) 


p = forest(
  (my_df %>%
     # mutate(across(everything(.),as.character)) %>%
     # add_row(!!!empty_row_values, .before = 4) %>%
    # mutate(spacer="") %>%
     select(C,n,`HR (95% CI, p-value)`,`CVD Death`,`HR (95% CI, p-value) `,`All-cause Death`) %>%
     rename(" "=C)
   
   ),
            est = list(my_df$est_gp1,
                       my_df$est_gp2,
                       my_df$est_gp3,
                       my_df$est_gp4
                     ),
            lower = list(my_df$low_gp1,
                         my_df$low_gp2,
                         my_df$low_gp3,
                         my_df$low_gp4), 
            upper = list(my_df$high_gp1,
                         my_df$high_gp2,
                         my_df$high_gp3,
                         my_df$high_gp4),
            ci_column = c(4, 6),
            ...
            theme = forest_theme(base_size = 10,
                                 core=list(bg_params=list(fill = c("white")),
                                           padding = unit(c(4, 10), "mm")
                                           ),
                   refline_lty = "solid",
                   ci_pch = (c(15, 15,15,15)),
                   ci_col = c("#DD002D", "#FFC16A", "#00DEBE","#9F54A8"),

                   footnote_col = "blue",
                   legend_name = "Group",
                   legend_value = c("Model 0", "Model 1", "Model 2", "Model 3"),
